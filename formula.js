...
class BinaryOperatorFormula extends Formula {
  ...
  isEqualTo(otherFormula) {
    if (otherFormula.type == "Binary") {
      if (this.operator == otherFormula.operator) {
        if (
          this.f1.isEqualTo(otherFormula.f1) &&
          this.f2.isEqualTo(otherFormula.f2)
        ) {
          return true;
        } else if (
          this.f1.isEqualTo(otherFormula.f2) &&
          this.f2.isEqualTo(otherFormula.f1)
        ) {
          return true;
        }
      }
  ...
  toTextual() {
    return (
      "(" + this.f1.toTextual() + this.operator + this.f2.toTextual() + ")"
    );
  }

  getSubFormulas() {
    return [this]
      .concat(this.f1.getSubFormulas())
      .concat(this.f2.getSubFormulas());
  }
}
